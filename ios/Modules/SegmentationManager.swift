...
@objc class SegmentationManager: NSObject {
    ...
    // MLMultiArray로부터 바이너리 마스크 이미지를 생성하는 함수 (공용)
    ...
    @objc func processImage(_ image: UIImage, completion: @escaping (UIImage?, Error?) -> Void) {
        ...
        // 모델 로드 및 시간 측정
        let modelLoadStartTime = CACurrentMediaTime()
        ...
        let modelLoadTime = CACurrentMediaTime() - modelLoadStartTime
        print("모델 로드 시간: \(modelLoadTime)초")
        ...
        print("모델 추론 시간: \(inferenceTime)초")
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            autoreleasepool {
                ...
                print("히트맵 생성 시간: \(heatmapTime)초")
                
                // 결과 이미지 오버레이 및 생성 시간 측정
                ...
                print("오버레이 및 결과 이미지 생성 시간: \(overlayTime)초")
                
                let stepTimeSum = resizeTime + modelLoadTime + convertTime + inferenceTime + heatmapTime + overlayTime
                print("단계별 시간 합계: \(stepTimeSum)초")
                print("전체 처리 시간: \(totalTime)초")
